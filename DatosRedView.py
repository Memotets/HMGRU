...
def  DatosRed_lista_nodos(request):
    ...
    if(request.method == 'POST'):
        ...
        ip = data['ip'] # ip del edificio que se va a consultar
        oids = data['oids'] # oids de los nodos del edificio a los que se les realizara el seguimiento
        previos = ast.literal_eval(data['previos']) # octetos leidos en la consulta anterior en forma de diccionario

        previos_entrada = 0 if (previos is None) else int(previos['entrada']) # lista de los octetos de entrada leidos en la conuslta anterior
        previos_salida =  0 if (previos is None) else int(previos['salida']) # lista de los octetos de sakida leidos en la consulta anterior

        octetos_entrada = [] # lista en la que se almacenaran los octetos de entrada leidos en la consulta a realizar
        octetos_salida = [] # lista en la que se almacenaran los octetos de salida leidos en la consulta a ralizar
        _ids = [] # lista de oids a los que se les calculo el consumo

        consumo_entrada = [] # lista del consumo de entrada de cada nodo en seguimiento
        consumo_salida = [] # lista del consuimo de salida de cada nodo en seguimiento

        datos_entrada, datos_salida = snmp(ip) # consulta de los octetos actuales en los nodos del edificio

        limit = len(datos_entrada) 
        for i in range(0, limit):
            key = '%i' %(i) # oid actual en la lista general

            """
                Se consulta si el oid actual se encuentra en la lista de 
